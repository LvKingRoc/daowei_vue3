<template>
  <div class="container">
      ...
      <el-table :data="currentPageCustomers" style="width: 100%" stripe border size="small" v-loading="loading"
        ...
        <el-table-column label="联系人信息" min-width="200" align="center">
          <template #default="{ row }">
            ...
            <div v-if="!row.contacts.length">-</div>
          </template>
        </el-table-column>
        <el-table-column label="地址" min-width="200" align="center">
          <template #default="{ row }">
            ...
            <div v-if="!row.addresses.length">-</div>
          </template>
        </el-table-column>
    ...
    <div class="pagination-container">
      <span class="pagination-text">总数：{{ totalCount }}条数据</span>
      ...
      <el-pagination v-model:current-page="currentPage" :page-size="pageSize" :total="totalCount" layout="pager"
      ...
      <el-input v-model="jumpPage" class="page-jumper" placeholder="页码" type="number" :min="1"
        :max="Math.ceil(totalCount / pageSize)" @keyup.enter="handleJumpPage" />
    ...
    <el-dialog v-model="showDeleteDialog" title="删除确认" width="500px" :close-on-click-modal="false">
      <div class="delete-warning-content">
        <div class="warning-title">
          ...
          <span style="margin-left: 10px;">确认删除以下客户？</span>
        </div>
        <div class="customer-info">
          <div class="info-item"><span class="label">企业名称：</span>{{ deleteItemData.companyName }}</div>
          <div class="info-item"><span class="label">联系人：</span>{{ deleteItemData.contactCount }}人</div>
          <div class="info-item"><span class="label">地址：</span>{{ deleteItemData.addressCount }}个</div>
        </div>
        <div class="warning-details">
          <div class="warning-title-sub">删除后相关样品、订单的"客户信息"将会同时删除：</div>
          <div class="warning-item">
            <i class="el-icon-document-delete"></i>
            <span>所有关联的样品数据（<span style="color: #f56c6c">{{ deleteItemData.sampleCount }}个样品</span>）</span>
          </div>
          <div class="warning-item">
            <i class="el-icon-document-delete"></i>
            <span>所有关联的订单数据（<span style="color: #f56c6c">{{ deleteItemData.orderCount }}个订单</span>）</span>
          </div>
        </div>
    ...
    <!-- 二次确认弹窗 -->
    <el-dialog v-model="showSecondConfirmDialog" title="二次确认" width="400px" :close-on-click-modal="false" center>
      <div class="second-confirm-content">
        ...
        <div class="warning-message">
          ...
          <div class="detail">该客户关联了 <span style="color: #f56c6c">{{ deleteItemData.sampleCount }}</span> 个样品</div>
          <div class="detail">该客户关联了 <span style="color: #f56c6c">{{ deleteItemData.orderCount }}</span> 个订单</div>
          <div class="detail">所有相关样品和订单的"客户信息"将被<span class="danger">永久删除</span></div>
          <div class="sub-detail">您确定要继续吗？</div>
        </div>
      </div>
      <template #footer>
        <el-button @click="showSecondConfirmDialog = false">返回</el-button>
...
const showSecondConfirmDialog = ref(false); // 二次删除确认弹窗
...
const deleteCustomer = async (customer) => {
  try {
    ...
    const [sampleResponse, orderResponse] = await Promise.all([
      request.get(`/customers/${customer.id}/samples/count`),
      request.get(`/customers/${customer.id}/orders/count`)
    ]);
...
const executeDelete = async () => {
  try {
    await request.delete(`/customers/${deleteItemData.value.id}`);
...
/**
 * 确认删除，根据关联数据判断是否需要二次确认
 */
const confirmDelete = () => {
  showDeleteDialog.value = false;
  // 如果有关联的样品或订单，显示二次确认弹窗
...
const validateAndSave = async () => {
  try {
    ...
    const url = isUpdate ? `/customers/${formData.value.id}` : '/customers';
    ...
    if (response && response.success) {
      ...
      ElMessage.error(response?.message || (isUpdate ? '更新失败' : '添加失败'));
    }
  } catch (error) {
    // 捕获验证失败或请求失败的错误
