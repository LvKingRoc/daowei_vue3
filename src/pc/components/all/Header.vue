...
import { EXTERNAL_APIS, API_KEYS } from '@/config/env.ts';
...
const selectedDayData = ref<CalendarData | null>(null);
...
const fetchTodayData = async () => {
  try {
    const response = await fetch(`${EXTERNAL_APIS.CALENDAR_API.BASE_URL}?id=${API_KEYS.CALENDAR_API.ID}&key=${API_KEYS.CALENDAR_API.KEY}`);
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  if (calendar && timeDisplay && 
      ...
      !timeDisplay.contains(event.target as Node)) {
