import ToolsIndex from '@/mp/components/view/tools/ToolsIndex.vue';
import ImageCompressor from '@/mp/components/view/tools/ImageCompressor.vue';
import FileCompressor from '@/mp/components/view/tools/CreateZIP.vue';
import FileExtractor from '@/mp/components/view/tools/ExtractZIP.vue';
import TextRecognition from '@/mp/components/view/tools/ImageToText.vue';
import AllAI from '@/mp/components/view/tools/AllAI.vue';

// 工具箱路由配置（扁平结构）
const createToolsRoutes = (role) => {
  const prefix = role === 'admin' ? 'Admin' : 'User';
  const basePath = `/${role}/tools`;

  return [
    {
      path: basePath,
      name: `${prefix}Tools`,
      component: ToolsIndex,
      meta: {
        title: '工具箱',
        requiresAuth: true,
        role,
        menuGroup: 'tools'
      }
    },
    {
      path: `${basePath}/image-compressor`,
      name: `${prefix}ImageCompressor`,
      component: ImageCompressor,
      meta: {
        title: '图片压缩工具',
        requiresAuth: true,
        role,
        menuGroup: 'tools',
        showBack: true
      }
    },
    {
      path: `${basePath}/text-recognition`,
      name: `${prefix}TextRecognition`,
      component: TextRecognition,
      meta: {
        title: '图片转文字',
        requiresAuth: true,
        role,
        menuGroup: 'tools',
        showBack: true
      }
    },
    {
      path: `${basePath}/file-compressor`,
      name: `${prefix}FileCompressor`,
      component: FileCompressor,
      meta: {
        title: '制作压缩包',
        requiresAuth: true,
        role,
        menuGroup: 'tools',
        showBack: true
      }
    },
    {
      path: `${basePath}/file-extractor`,
      name: `${prefix}FileExtractor`,
      component: FileExtractor,
      meta: {
        title: '解压压缩包',
        requiresAuth: true,
        role,
        menuGroup: 'tools',
        showBack: true
      }
    },
    {
      path: `${basePath}/AllAI`,
      name: `${prefix}AllAI`,
      component: AllAI,
      meta: {
        title: 'AI大全',
        requiresAuth: true,
        role,
        menuGroup: 'tools',
        showBack: true
      }
    }
  ];
};

// 创建用户和管理员的工具箱路由（展开数组）
const toolsRoutes = [
  ...createToolsRoutes('user'),
  ...createToolsRoutes('admin')
];

export default toolsRoutes;
